<div x-data="{ open: false, focusIndex: -1 }" class="relative inline-block text-left" @mouseenter="open = true" @mouseleave="open = false" @keydown.window.escape="open = false">
    <div>
        <button type="button" class="inline-flex w-full justify-center gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 ring-1 shadow-xs ring-gray-300 ring-inset hover:bg-gray-50" id="menu-button" aria-expanded="true" aria-haspopup="true" @click="open = !open" @keydown.arrow-down.prevent="open = true; focusIndex = 0">
            {{ $slot }}
    ...
    <div x-show="open" class="absolute right-0 z-10  w-56 origin-top-right rounded-md bg-white ring-1 shadow-lg ring-black/5 focus:outline-hidden" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabindex="-1" @mouseenter="open = true" @mouseleave="open = false" @keydown.arrow-down.prevent="focusIndex = (focusIndex + 1) % 3" @keydown.arrow-up.prevent="focusIndex = (focusIndex - 1 + 3) % 3" @keydown.enter.prevent="document.getElementById('menu-item-' + focusIndex).click()">
        <div class="py-1" role="none">
            <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="menu-item-0" :class="{ 'bg-gray-100': focusIndex === 0 }" @mouseenter="focusIndex = 0">Pizzas</a>
            <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="menu-item-1" :class="{ 'bg-gray-100': focusIndex === 1 }" @mouseenter="focusIndex = 1">Pasta</a>
            <a href="#" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1" id="menu-item-2" :class="{ 'bg-gray-100': focusIndex === 2 }" @mouseenter="focusIndex = 2">Hamburguesas</a>
        </div>
    </div>
</div>
